...
func createDatabaseIfNotExists(mc MySQLConfig) {
	...
	adminDSN := fmt.Sprintf("%s:%s@tcp(%s)/?charset=utf8mb4&parseTime=True&loc=Local&tls=false",
...
func connectWithRetry(mc MySQLConfig) {
	dsn := fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8mb4&parseTime=True&loc=Local&tls=false",
